...
const App: React.FC = () => {
  ...
  const calibrationLogs = useLiveQuery(() => db.logs.orderBy('timestamp').reverse().toArray()) ?? [];
  ...
  const handleSubmit = useCallback(async (data: ObservationFormData) => {
    ...
    if (!data.observationText.trim() && (data.images?.length || 0) === 0) {
     ...
     if (apiSettings.provider === 'doubao' && (!apiSettings.doubaoApiKey?.trim() || !apiSettings.doubaoModelEndpoint?.trim())) {
      setError("请先在右上角“设置”中配置豆包模型的 API Key 和 Endpoint。");
      setIsSettingsOpen(true);
      return;
    }
    if (apiSettings.provider === 'siliconflow' && (!apiSettings.siliconApiKey?.trim() || !apiSettings.siliconModel?.trim())) {
