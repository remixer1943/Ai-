...
const getApiSettings = (): ApiSettings => {
  try {
    ...
    return savedSettings ? { ...defaultApiSettings, ...JSON.parse(savedSettings) } : defaultApiSettings;
  } catch (error) {
...
export const extractEvidence = async (
    ...
    if (settings.provider === 'doubao' && settings.doubaoApiKey && settings.doubaoModelEndpoint) {
        return doubao.extractEvidence(observationText, settings.doubaoApiKey, settings.doubaoModelEndpoint);
    }
    if (settings.provider === 'siliconflow' && settings.siliconApiKey && settings.siliconModel) {
        return silicon.extractEvidence(observationText, settings.siliconApiKey, settings.siliconModel, settings.siliconBaseUrl);
    }
    // Default to Gemini
...
export const queryGuide = async (
    ...
    if (settings.provider === 'siliconflow' && settings.siliconApiKey && settings.siliconModel) {
        return silicon.queryGuide(data, settings.siliconApiKey, settings.siliconModel, settings.siliconBaseUrl, calibrationLogs);
    }
    // Default to Gemini
...
// Meta-analysis is a complex task.
export const getGrowthInsightAnalysis = async (log: Pick<CalibrationLog, 'originalText' | 'aiInitialEvidence' | 'calibratedEvidence'>): Promise<string> => {
    const settings = getApiSettings();
    if (settings.provider === 'doubao' && settings.doubaoApiKey && settings.doubaoModelEndpoint) {
        return doubao.getGrowthInsightAnalysis(log, settings.doubaoApiKey, settings.doubaoModelEndpoint);
    }
    if (settings.provider === 'siliconflow' && settings.siliconApiKey && settings.siliconModel) {
        return silicon.getGrowthInsightAnalysis(log, settings.siliconApiKey, settings.siliconModel, settings.siliconBaseUrl);
    }
...
export async function* generateLearningStoryStream(
    ...
    // Route to Doubao
    if (settings.provider === 'doubao' && settings.doubaoApiKey && settings.doubaoModelEndpoint) {
        yield* doubao.generateLearningStoryStream(observationText, analysisResult, settings.doubaoApiKey, settings.doubaoModelEndpoint);
        return;
    }
    if (settings.provider === 'siliconflow' && settings.siliconApiKey && settings.siliconModel) {
