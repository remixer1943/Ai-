...
import {
  ...
  VideoKeyframePayload,
} from '../components/ObservationForm';
import { apiFetch } from './apiClient';
import { getRelevantExamples } from './feedbackService';
import { getCalibrationRules } from './guideService';
import { GoogleGenAI, Type } from '@google/genai';

// Ensure API_KEY environment variable is set
if (!process.env.API_KEY) {
  console.warn(
    'API_KEY environment variable not set. Please ensure it is available in the execution context.'
  );
}
...
const matchedTargetSchema = {
  ...
  properties: {
    ...
    title: {
      type: Type.STRING,
      description:
        "《指南》中针对特定年龄段的**具体行为指标**。例如: '喜欢倾听各种好听的声音, 感知声音的高低、长短、强弱等变化。'",
    },
    source: {
      type: Type.STRING,
      description:
        "目标的**完整层级路径**，用于教师快速定位。**必须**严格遵循格式：'领域 / 方面 / 目标X 标题 / 年龄段'。例如：'艺术 / 感受与欣赏 / 目标1 喜欢自然界与生活中美的事物 / 4-5岁'。**此字段用于导航，title字段用于显示，二者内容不同。**",
    },
    evidence: {
      type: Type.STRING,
      description:
        'A direct quote or a very close paraphrase from the original observation text that supports this match.',
    },
    reasoning: {
      type: Type.STRING,
      description:
        "A brief analysis (1-2 sentences) explaining why the provided 'evidence' supports the matched 'title', helping the teacher understand the connection.",
    },
    confidence: {
      type: Type.NUMBER,
      description: 'A value between 0 and 1, representing the confidence in this match.',
    },
    ...
    educationalSuggestions: {
      ...
      items: { type: Type.STRING },
      description:
        'An array of 1-3 concrete educational suggestions from the guideline document that correspond to this matched target.',
    },
  },
  required: [
    'id',
...
const queryResultSchema = {
  ...
  properties: {
    queryId: {
      type: Type.STRING,
      description: "A unique identifier for this query, starting with 'qry-'.",
    },
    inputText: {
      type: Type.STRING,
      description: 'The original observation text provided by the user.',
    },
    domainPrediction: {
      ...predictionSchema,
      description: 'The primary developmental domain predicted.',
    },
    agePrediction: {
      ...predictionSchema,
      description: 'The predicted age group based on the observation.',
    },
    holistic_evaluation: {
      type: Type.STRING,
      description: `
        对幼儿整体发展的专业综合评价（200-300字）。必须包含：
        1. 核心发展特征（基于具体行为证据）
        2. 理论视角分析（如认知阶段、学习品质等）
        3. 发展亮点与潜能
        4. 温和的支持建议（不超过2条）
...
- JSON 输出必须符合提供的 schema，禁止额外字段或解释。`;
  ...
  try {
    ...
    try {
      ...
      console.error('Failed to parse video observation JSON:', responseText, error);
...
const retrieveKnowledge = async (query: string, topK: number = 5): Promise<string> => {
  ...
  try {
    const data = await apiFetch<RAGResponse>(
      ...
      { retries: 2, retryDelayMs: 600, timeoutMs: 10000 }
    );
...
export const queryGuide = async (
  ...
  // Retrieve relevant examples from feedback logs
  const relevantExamples = await getRelevantExamples();

  // Retrieve learned calibration rules
  const calibrationRules = await getCalibrationRules();
...
2.  **循证分析**：'evidence'字段必须是直接引用或高度概括观察记录中的原话，这是证明你的匹配是循证的、而不是凭空捏造的关键。如果提供了图片或视频，你的分析和证据需要结合多模态内容。
3.  **图片分析规则 (如果提供了图片)**:
    a.  **视觉证据引用**: 你必须仔细观察图片中的细节（如儿童的表情、动作、手部精细动作、使用的材料结构等）。
    b.  **显式引用**: 在'evidence'字段中，必须明确指出证据来源。例如使用“如图所示，孩子...”或“图片中显示...”这样的措辞。
    c.  **多模态互证**: 将图片看到的细节与文字记录相互印证。如果图片展示了文字未描述的细节（例如积木搭建的具体结构），请务必补充到证据中。
4.  **视频分析规则 (如果提供了视频)**:
...
5.  **【强制】核心预测**：判断观察记录最能体现的核心发展领域（'domainPrediction'），并给出置信度（0到1之间）。**重要约束**：'domainPrediction.value' 字段**必须且只能**是以下五大领域之一：**健康**、**语言**、**社会**、**科学**、**艺术**。严禁使用其他值（如"指南"、"综合"等）。
6.  **【强制】多维分析**: 一段复杂的观察记录（或一条关键证据）可能同时反映了幼儿在多个领域或多个目标上的发展。如果适用，你应当为同一段证据匹配多个来自不同领域或方面的目标。不要将你的分析局限于单一视角。
7.  **匹配发展目标**：找出所有相关的具体发展目标（'matchedTargets'），并按置信度从高到低排序，最多不超过5个。
8.  **提供观察建议**：为每个匹配的目标提供2条具体的、可操作的"即时观察项"（'suggested_observations'），帮助教师进行下一步观察。
9.  **提供教育建议**：对于每个匹配的目标，**必须**从《指南》或自定义知识库中找出对应的"教育建议"，并将其放入'educationalSuggestions'数组中。
10. **【强制】提供解析**：为每个匹配的目标，你**必须**提供一个'reasoning'字段。该字段应包含简洁的分析（1-2句话），解释'evidence'与匹配的'title'之间的逻辑联系，帮助教师理解你的思路。
11. **【强制】来源路径格式**: 'source' 字段至关重要，它为教师提供了在《指南》中回溯的完整路径。因此，它**必须**严格遵循 '领域 / 方面 / 目标X 标题 / 年龄段' 的格式。例如，一个正确的'source'是 '科学 / 科学探究 / 目标2 具有初步的探究能力 / 4-5岁'。
12. **【新增】整体评价 (holistic_evaluation)**：
...
13. **JSON输出**：你的回答必须是严格遵循所提供schema的单个JSON对象，不要包含任何额外的解释、注释或markdown标记。

**请在返回的 JSON 中的 holistic_evaluation 字段提供整体评价，遵循上述规则。**`;
...
${goldenExample}`;
  ...
  const userPrompt = `
    ...
    ${images && images.length > 0 ? `- **图片附件**: 包含 ${images.length} 张现场照片，请务必结合图片中的视觉细节进行分析。` : ''}
    ${combinedKnowledgeBase ? `- **参考资料 (Reference Knowledge)**: """${combinedKnowledgeBase}"""` : ''}
    ${relevantExamples ? `\n    **【重要】参考范例 (来自教师的历史反馈)**:\n    ${relevantExamples}\n    请参考上述范例的分析风格和深度。` : ''}
    ${calibrationRules ? `\n    **【重要】教师的证据提取原则 (从历史校准中学习)**:\n    ${calibrationRules}\n    请务必遵守上述原则来提取证据。` : ''}
