...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ results, isLoading, loadingStage, error, onFeedback, feedbackSent }) => {
  ...
  const showAgeMismatchWarning = !isAutoDetectMode && hasConfidentAgePrediction && results.selectedAgeGroup && results.selectedAgeGroup !== aiAgeValueRaw;
      ...
      {isAutoDetectMode && (
        <div className="flex items-start gap-3 mb-6 p-3 bg-blue-50 dark:bg-slate-700/50 border border-blue-200 dark:border-blue-700/60 rounded-lg text-sm text-blue-800 dark:text-blue-200">
          ...
          <p>
            ...
            根据您的观察记录，AI判断孩子的行为表现最符合 ‘<strong>{aiAgeDisplayValue}</strong>’，并以此为标准进行了分析。
          </p>
        </div>
      )}
