...
export const ObservationForm: React.FC<ObservationFormProps> = ({ onSubmit, isLoading, isResultView, onNewQuery }) => {
    ...
    const videoMetadataSummary = video ? `${formatDuration(video.duration)} · ${video.width}×${video.height} · ${(video.size / (1024 * 1024)).toFixed(1)} MB` : '';
    const videoProcessingSummary = video
        ? (isVideoPreparing
            ? '正在解析视频...'
            : isGeneratingObservation
                ? 'AI 正在撰写观察记录...'
                : (videoStatusMessage || (videoObservationDraft ? 'AI 已生成结构化草稿。' : '视频已就绪，可生成 AI 草稿。')))
        : '';
        ...
        <form onSubmit={handleSubmit} className="relative space-y-6 animate-fade-in font-['Noto_Sans_SC']">
            ...
            {isResultView && (
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">分析报告</h2>
                    ...
                    <div className="px-6 py-4 bg-slate-50/80 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700/60 flex flex-wrap justify-end gap-3 backdrop-blur-md">
                        {isCalibrating ? (
                            <div className="flex-1 flex items-center justify-between mr-4 animate-fade-in bg-indigo-50 dark:bg-indigo-900/20 px-4 py-2 rounded-xl border border-indigo-100 dark:border-indigo-900/30">
                                ...
                                <div className="flex gap-2">
                                    <button type="button" onClick={handleAddHighlight} className="text-xs px-3 py-1.5 bg-white dark:bg-slate-800 border border-indigo-200 dark:border-slate-600 rounded-lg text-indigo-700 dark:text-indigo-300 hover:shadow-sm font-semibold">手动高亮</button>
                                    <button type="button" onClick={() => setIsCalibrating(false)} className="text-xs px-3 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-sm font-semibold">完成校准</button>
                                </div>
                            </div>
                        ) : (
                            <>
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-3xl p-6 shadow-lg shadow-indigo-100/30 dark:shadow-none border border-white/50 dark:border-slate-700 transition-all hover:bg-white/80 dark:hover:bg-slate-800/80">
                            ...
                            <p className="text-xs text-slate-400 mt-3 text-right font-medium">{images.length}/5 张</p>
                        </div>
                        ...
                        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-3xl p-6 shadow-lg shadow-indigo-100/30 dark:shadow-none border border-white/50 dark:border-slate-700 transition-all hover:bg-white/80 dark:hover:bg-slate-800/80">
                            ...
                            {!video ? (
                                ...
                                >
                                    ...
                                    <span className="text-sm text-slate-500 dark:text-slate-400 font-medium group-hover:text-indigo-600 dark:group-hover:text-indigo-400">点击上传视频片段</span>
                                ...
                                <>
                                    <div className="mt-3 space-y-1 text-xs text-slate-500 dark:text-slate-400 bg-white/50 dark:bg-slate-900/30 rounded-2xl p-3 border border-slate-100 dark:border-slate-800">
                                        ...
                                        {video.keyframes.length > 0 && (
                                            <div>已自动截取 {video.keyframes.length} 张关键帧</div>
                                        )}
                                    ...
                                    {videoObservationDraft && (
                                        <div className="mt-3 text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-900/20 rounded-2xl p-4 border border-slate-100 dark:border-slate-800 space-y-4">
                                            <div>
                                                <div className="font-semibold text-slate-700 dark:text-slate-200 mb-1">AI 关键瞬间</div>
                                                <ul className="list-disc list-inside space-y-1">
                                                    {videoObservationDraft.key_moments.map((moment, index) => (
                                                        <li key={`${moment.timecode}-${index}`}>
                                                            ...
                                                            {moment.strategy && <span className="text-slate-500 dark:text-slate-400">（策略：{moment.strategy}）</span>}
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                                            <div>
                                                <div className="font-semibold text-slate-700 dark:text-slate-200 mb-1">学习品质</div>
                                                <ul className="list-disc list-inside space-y-1">
                                                    {videoObservationDraft.learning_traits.map((trait, index) => (
                                                        <li key={`${trait.trait}-${index}`}><span className="font-medium">{trait.trait}</span>：{trait.description}</li>
                                                    ))}
                                                </ul>
                                            </div>
                                            <div>
                                                <div className="font-semibold text-slate-700 dark:text-slate-200 mb-1">指标映射</div>
                                                <ul className="list-disc list-inside space-y-1">
                                                    {videoObservationDraft.targets.map((target, index) => (
                                                        <li key={`${target.indicator}-${index}`}>
                                                            ...
                                                            <span className="text-slate-500 dark:text-slate-400"> —— {target.evidence}{target.timecode ? `（${target.timecode}）` : ''}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            </div>
                                            <div>
                                                <div className="font-semibold text-slate-700 dark:text-slate-200 mb-1">分析</div>
                                                <p className="leading-relaxed text-slate-600 dark:text-slate-300">{videoObservationDraft.analysis}</p>
                                            </div>
                                            <div>
                                                <div className="font-semibold text-slate-700 dark:text-slate-200 mb-1">下一步支架</div>
                                    ...
                                    >
                                        ...
                                        {isGeneratingObservation ? 'AI 正在观看视频...' : '重新生成 AI 观察'}
                                    </button>
                                </>
                            )}
                        </div>
                        ...
                        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-3xl p-6 shadow-lg shadow-indigo-100/30 dark:shadow-none border border-white/50 dark:border-slate-700 transition-all hover:bg-white/80 dark:hover:bg-slate-800/80">
                            ...
                            {!knowledgeFile ? (
                                ...
                                >
                                    <span className="group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">点击上传 .pdf / .docx</span>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 pt-8 mt-2">
                        ...
                        >
                            <span className="relative z-10 flex items-center justify-center gap-3 text-base">
                                {isLoading ? '深度思考中...' : isParsing ? '文件处理中...' : isHighlighting ? '提取证据中...' : '开始智能分析'}
